...
void run_cplex(Timer& timer) {
    ...
    try{
        ...
        // create a model and link it to the environment env
        IloModel model(env);

        // create an array of binary variables, 
        // one for each node of the input graph with lower bound 0, upper bound 1 and type ILOINT
		// it is an array that contains all the nodes
        IloNumVarArray x(env, n_of_nodes, 0, 1, ILOINT);
    
        // create an expression obj
        IloExpr obj(env);

        // add all node variables to the expression obj
        ...
        for (int i = 0; i < n_of_nodes; ++i) {
			...
			model.add(expr >= 0.5 * neighbors[i].size());
